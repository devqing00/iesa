...
const LEVEL_COLORS: Record<string, { bg: string; dot: string; active: string }> = {
  All: { bg: "bg-navy", dot: "bg-lime", active: "text-snow" },
...
export default function ClassRepsPage() {
  ...
  const { showHelp, openHelp, closeHelp } = useToolHelp("team-class-reps");
  ...
  return (
    <div className="space-y-6">
      <ToolHelpModal toolId="team-class-reps" isOpen={showHelp} onClose={closeHelp} />
      {/* ── section header ── */}
                ...
                return (
                  <div key={level}>
                    <div className="flex items-center gap-3 mb-4">
                      ...
                      <span className="text-body font-bold text-xs text-slate">({reps.length} rep{reps.length !== 1 ? "s" : ""})</span>
...
function RepCard({ rep, accent, index }: { rep: ClassRep; accent: typeof ACCENT_CYCLE[0]; index: number }) {
  ...
  return (
    <div className={`bg-snow border-[3px] border-navy border-l-[6px] ${accent.border} rounded-[1.5rem] p-5 shadow-[4px_4px_0_0_#000] ${rotations[index % 4]} hover:rotate-0 transition-transform`}>
