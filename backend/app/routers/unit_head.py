...
Endpoints:
  GET    /api/v1/unit-head/my-units                    — Which units this user heads
  GET    /api/v1/unit-head/{unit}/members               — Member roster
  GET    /api/v1/unit-head/{unit}/noticeboard           — List noticeboard posts
  POST   /api/v1/unit-head/{unit}/noticeboard           — Create noticeboard post
  PUT    /api/v1/unit-head/{unit}/noticeboard/{id}      — Update post
  DELETE /api/v1/unit-head/{unit}/noticeboard/{id}      — Delete post
  GET    /api/v1/unit-head/{unit}/tasks                 — List tasks
  POST   /api/v1/unit-head/{unit}/tasks                 — Create task
  PATCH  /api/v1/unit-head/{unit}/tasks/{id}            — Update task (status, details)
  DELETE /api/v1/unit-head/{unit}/tasks/{id}            — Delete task
  POST   /api/v1/unit-head/{unit}/announce              — Send unit-targeted announcement
  GET    /api/v1/unit-head/{unit}/analytics             — Activity stats + task completion rates
  GET    /api/v1/unit-head/{unit}/admin-content         — Admin view of noticeboard + tasks

Member-facing (authenticated, no head role):
  GET    /api/v1/unit-head/my-memberships               — Which units this user belongs to
  GET    /api/v1/unit-head/{unit}/member-view            — Member view (noticeboard + tasks)
  PATCH  /api/v1/unit-head/{unit}/tasks/{id}/status     — Member updates own task status
"""
...
router = APIRouter(prefix="/api/v1/unit-head", tags=["unit-head"])
...
async def _get_user_headed_units(user_id: str, session_id: str, db) -> list[dict]:
    """Return list of {unitSlug, unitLabel, isCustom} for units this user heads."""
    ...
    # Static units: check HEAD_POSITION_TO_UNIT
    for pos in positions:
        unit_slug = HEAD_POSITION_TO_UNIT.get(pos)
        if unit_slug:
            result.append({
                "unitSlug": unit_slug,
                "unitLabel": UNIT_LABELS.get(unit_slug, unit_slug),
                "isCustom": False,
            })
...
async def _verify_head_of_unit(user_id: str, unit_slug: str, session_id: str, db) -> dict:
    """Verify user is head of the given unit. Returns unit info dict or raises 403."""
    headed = await _get_user_headed_units(user_id, session_id, db)
    match = next((u for u in headed if u["unitSlug"] == unit_slug), None)
    if not match:
        raise HTTPException(403, "You are not the head of this unit")
    return match


async def _get_unit_member_ids(unit_slug: str, session_id: str, db) -> list[str]:
    """Return list of userIds who are active members of this unit."""
    role_info = UNIT_ROLE_MAP.get(unit_slug)
    if not role_info:
        # Custom unit: no members via role system (custom units don't have UNIT_ROLE_MAP entries)
        return []
...
async def _get_unit_members_full(unit_slug: str, session_id: str, db) -> list[dict]:
...
# ═══════════════════════════════════════════════════════════
# HEAD ENDPOINTS (require unit_head:* permissions)
# ═══════════════════════════════════════════════════════════

@router.get("/my-units")
async def my_headed_units(
    user=Depends(require_permission("unit_head:view_members")),
    ...
    return await _get_user_headed_units(user_id, session_id, db)


@router.get("/{unit}/members")
async def unit_members(
    ...
    user_id = str(user.get("_id") or user.get("id"))
    session_id = str(session["_id"])
    unit_info = await _verify_head_of_unit(user_id, unit, session_id, db)
    members = await _get_unit_members_full(unit, session_id, db)
    return {"unit": unit_info, "members": members}


# ── Noticeboard ──────────────────────────────────────────────

@router.get("/{unit}/noticeboard")
async def list_notices(
    unit: str,
    user=Depends(require_permission("unit_head:manage_noticeboard")),
    session=Depends(get_current_session),
):
    db = get_database()
    user_id = str(user.get("_id") or user.get("id"))
    session_id = str(session["_id"])
    await _verify_head_of_unit(user_id, unit, session_id, db)
...
@router.put("/{unit}/noticeboard/{post_id}")
...
@router.delete("/{unit}/noticeboard/{post_id}")
...
@router.get("/{unit}/tasks")
async def list_tasks(
    unit: str,
    status: Optional[str] = Query(None),
    user=Depends(require_permission("unit_head:manage_tasks")),
    session=Depends(get_current_session),
):
    db = get_database()
    user_id = str(user.get("_id") or user.get("id"))
...
async def create_task(
    ...
    member_ids = await _get_unit_member_ids(unit, session_id, db)
    ...
    if notify_ids:
        fire_and_forget(create_bulk_notifications(
            ...
            message=f"[{unit_info['unitLabel']}] {body.title}",
...
@router.patch("/{unit}/tasks/{task_id}")
...
@router.delete("/{unit}/tasks/{task_id}")
...
@router.post("/{unit}/announce")
async def create_unit_announcement(
    unit: str,
    body: AnnouncePayload,
    user=Depends(require_permission("unit_head:announce")),
    session=Depends(get_current_session),
):
    ...
    announcement = {
        ...
        "priority": body.priority,
        "targetUnit": unit,
        "targetUnitLabel": unit_info["unitLabel"],
        "author": user_name,
        "authorId": user_id,
        "sessionId": session_id,
        "createdAt": now,
    ...
    if member_ids:
        fire_and_forget(create_bulk_notifications(
            user_ids=member_ids,
            type="announcement",
            title=f"[{unit_info['unitLabel']}] {body.title}",
...
async def get_unit_analytics(
    ...
    Available to the unit head (requires unit_head:view_members permission).
    """
    db = get_database()
    user_id = str(user.get("_id") or user.get("id"))
    session_id = str(session["_id"])
...
@router.get("/{unit}/admin-content")
async def admin_unit_content(
    unit: str,
    ...
    Requires unit_application:review permission (admin/exco with review rights).
    """
    db = get_database()
    session_id = str(session["_id"])
...
async def my_memberships(
    ...
    # Build a reverse map: position → unitSlug
    position_to_unit = {}
    for slug, info in UNIT_ROLE_MAP.items():
    ...
    for r in roles:
        ...
        if slug:
            # Get head info
            head_position = UNIT_TO_HEAD_POSITION.get(slug)
            head = None
            if head_position:
            ...
            # Count members
            role_info = UNIT_ROLE_MAP.get(slug, {})
            member_count = 0
            ...
            result.append({
                "unitSlug": slug,
                "unitLabel": UNIT_LABELS.get(slug, slug),
                "head": head,
                "memberCount": member_count,
                "joinedAt": r.get("createdAt"),
            })
...
async def member_view(
    ...
    if not _check_membership(user_id, member_ids):
        raise HTTPException(403, "You are not a member of this unit")
...
@router.patch("/{unit}/tasks/{task_id}/status")
